...
pdf.ln(10)
summary_text = "This report presents an overview of project delays based on the provided baseline and update schedules. It includes delay metrics, graphical insights, and narrative summaries for arbitration or executive review."
...
if baseline_df is not None and update_dfs:
    ...
    for i, update_df in enumerate(update_dfs):
        ...
        merged = pd.merge(baseline_df, update_df, on="activity id", suffixes=("_baseline", f"_update{i+1}"))
        merged[f"start_delay_update{i+1}"] = (pd.to_datetime(merged[f"start date_update{i+1}"], errors='coerce') - pd.to_datetime(merged["start date_baseline"], errors='coerce')).dt.days
        merged[f"finish_delay_update{i+1}"] = (pd.to_datetime(merged[f"finish date_update{i+1}"], errors='coerce') - pd.to_datetime(merged["finish date_baseline"], errors='coerce')).dt.days
        ...
        fig = px.bar(delay_report, x="activity id", y=[f"start_delay_update{i+1}", f"finish_delay_update{i+1}"], title=f"Delay Distribution – Update {i + 1}", labels={"value": "Days", "variable": "Delay Type"}, barmode="group")
